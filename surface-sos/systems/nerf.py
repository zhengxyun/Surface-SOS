...
class NeRFSystem(BaseSystem):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.save_image_grid(f"it{self.global_step}-{batch['index'][0].item()}-{'%.3f' % (psnr)}.png", [
        ...
        # com_np = opa_np[:, :, np.newaxis] * rgb_np
        #
        # self.save_res_image(f"res_nerf/{self.global_step}/alpha/{batch['index'][0].item()}.png", opa_np, (0, 1), None)
        # self.save_rgb_image(f"it{self.global_step}-val/{batch['index'][0].item()}_out_bbk.png", out_bbk_np, 'HWC',(0, 1))
        # self.save_grayscale_image(f"it{self.global_step}-val/{batch['index'][0].item()}_opa.png", opa_np, (0, 1), None)
        # # self.save_grayscale_image(f"it{self.global_step}-val/{batch['index'][0].item()}_dep.png", dep_np, (0, 1), None)
        # self.save_rgb_image(f"it{self.global_step}-val/{batch['index'][0].item()}_com.png", com_np, 'HWC', (0, 1))
        #
        #
        ...
        #
        # out_fgr_pth = os.path.join(self.config.res_dir, f"res_nerf/{self.global_step}/com_rgba")
        ...
        # comp_pth = os.path.join(self.config.res_dir, f"res_nerf/{self.global_step}/comp")
