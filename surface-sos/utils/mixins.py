...
class SaverMixin():
    ...
    def get_res_path(self, filename):
        res_path = os.path.join(self.res_dir, filename)
        # print(res_path)
        # print(os.path.dirname(res_path))
    ...
    # 230912
    def save_res_image(self, filename, img, data_range=DEFAULT_GRAYSCALE_KWARGS['data_range'], cmap=DEFAULT_GRAYSCALE_KWARGS['cmap']):
        img = self.get_grayscale_image_(img, data_range, cmap)
        cv2.imwrite(self.get_res_path(filename), img)
    ...
    def save_file(self, filename, src_path):
        ...
        shutil.copyfile(src_path, self.get_res_path(filename))
